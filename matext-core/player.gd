...
var camera_mode = 0
var view_mode_dir = Vector3.ZERO
...
var json_conv_cache = {}
func json_conv(title, data, get_in_cache):
	if get_in_cache and [title, data] in json_conv_cache:
		return json_conv_cache[[title, data]]
	else:
		var output = ['| ', title, ' >_\nv\n']
		for item in data:
			output.append('• ' + item + ' : ' + str(data[item]) + '\n')
		if get_in_cache:
			json_conv_cache[[title, data]] = ''.join(output)
		return ''.join(output)
	...
	if terminal_on:
		...
		if terminal_open == 'inventory':
			...
			if ray.is_colliding():
				...
				scanner_id_get = {}
			terminal_display.text = json_conv("OBJECT SCANNER", scanner_id_get['status'] if 'status' in scanner_id_get else '', false) + ('\n________________________________,,,\n' + '• press the object to enter its terminal' if 'menu' in scanner_id_get else '')
			if 'menu' in scanner_id_get and Input.is_action_just_pressed("click"):
				terminal_open = 'terminal_access' 

		elif terminal_open == 'help':
			terminal_display.text = json_conv("TERMINAL HELP", terminal_help, true)

		elif terminal_open == 'terminal_access':
			terminal_display.text = json_conv("TERMINAL ACCESS", scanner_id_get['menu'], true)
			...
			terminal_display.text = json_conv("OBJECT STATUS", scanner_id_get['status'] if 'status' in scanner_id_get else '', false)

		elif terminal_open == 'display_costum':
			terminal_display.text = terminal_costum_display
		else:
			terminal_display.text = json_conv('WELCOME TO THE TERMINAL', terminal_menu, true)
			get_input_terminal()
	else:
...
func camera_input(event):
	if camera_move and event is InputEventMouseMotion:
		...
		view_mode_dir.y -= event.relative.x * camera_sens
		if camera_dir.y > 360 or camera_dir.y < -360:
			camera_dir.y = 0
		elif view_mode_dir.y > 360 or view_mode_dir.y < -360:
			view_mode_dir.y = 0
	if camera_mode == 1 or camera_mode == 2:
		if event is InputEventMouseButton and event.pressed:
			if event.button_index == MOUSE_BUTTON_WHEEL_DOWN:
				camera_set.position.z += 1
			elif event.button_index == MOUSE_BUTTON_WHEEL_UP:
				camera_set.position.z -= 1
		elif camera_set.position.z == 0:
			camera_mode = 3
func camera():
	if camera_move:
		...
		if camera_mode == 2:
			if Input.is_action_pressed("ui_up"):
				...
				view_mode_dir.y = 0
				camera_handle.rotation_degrees.y = view_mode_dir.y
			else:
				camera_handle.rotation_degrees.y = view_mode_dir.y
		else:
			self.rotation_degrees.y = camera_dir.y
...
func _process(delta: float) -> void:
	camera()
	if terminal_open == 'inventory':
		terminal_display.text = json_conv("INVENTORY", inventory, false)
